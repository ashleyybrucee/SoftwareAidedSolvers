...
(define (ver impl spec)
  ...
  ;----------
  ; '=' compares numbers and we have a list
  ; 'eq?' cannot recursively compare contents of compound data type
  ; have to use 'equal?' because we have a list
)


; FIXME: The BV synthesizer.
; Have to save this to file THEN call it
(define (syn impl spec)
  (define-symbolic* in int32? [(procedure-arity spec)])
  ; my code
  ;----------
   (print-forms (synthesize #:forall (list in)
               #:guarantee (assert (equal? (apply impl in) (apply spec in)))))
  ;----------
  ; Have to bind the symbolic variables, but ?? are in a list
  ; 'apply' applies procedure to contents of a list
  ; 'apply' applies a function to a 'list' of variables
  ; because we don't have an actual list we are passing, just a list of variables
  ; that's why apply works
)
...
;def/dbg marks a procedure as part of the code to be debugged
;bvmax1 is what we get after debugging
(def/dbg bvmax0 
  ([0 r0] [1 r1]) 
  (2 bvsge 0 1)
  (3 bvneg 2)
